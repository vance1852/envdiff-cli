 ...
 */
export function lintEnvFile(filePath) {
 ...
 */
export function compareEnvFiles(filePaths) {
    ...
    allKeys.forEach((key) => {
        ...
        if (missingFiles.length === 0) {
            ...
            const quoteConfigs = Object.entries(entriesByFile).map(([file, entry]) => ({
                file,
                hasQuotes: entry.hasQuotes,
                quoteType: entry.quoteType,
                value: entry.value,
            }));
            const numericLikeValues = quoteConfigs.filter((c) => !isNaN(Number(c.value)));
            if (numericLikeValues.length > 1 && numericLikeValues[0]) {
                const firstConfig = numericLikeValues[0];
                const hasMismatch = !numericLikeValues.every((c) => c.hasQuotes === firstConfig.hasQuotes &&
