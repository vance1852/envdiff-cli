 ...
 */
export function formatLint(result, format = "table") {
    if (format === "json") {
    ...
    result.issues.forEach((issue) => {
        ...
        lines.push(`| ${issue.type} | ${issue.key} | ${issue.type === "duplicate_key" ? issue.lineNumbers.join(",") : issue.lineNumber} | ${details} |`);
    });
    return lines.join("\n");
}
/**
 ...
 * @returns Formatted string output
 */
export function formatCompare(result, format = "table") {
    if (format === "json") {
        return JSON.stringify(result, null, 2);
    }
    if (result.issues.length === 0) {
    ...
    result.issues.forEach((issue) => {
        ...
        switch (issue.type) {
            case "missing_variable":
                ...
                break;
            case "value_mismatch":
                const valueStrs = Object.entries(issue.valuesByFile).map(([file, val]) => `${file}: "${val}"`);
                details = `Values mismatch: { ${valueStrs.join(", ")} }`;
                break;
            case "format_mismatch":
                const formatStrs = Object.entries(issue.detailsByFile).map(([file, cfg]) => `${file}: hasQuotes=${cfg.hasQuotes}, quoteType=${cfg.quoteType}`);
